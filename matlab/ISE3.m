function [TR, TT, ER, t] = ISE3(q,p,varargin)
% Perform the Iterative Closest Point algorithm on three dimensional point
% clouds. This code is written by Matanya Horowitz, and based on research
% with Nikolai Matni
%
% This ICP code borrows heavily from the ICP Matlab package. We thank those authors at:
...
% Lambda
%       {1}
%       A real number describing the L1 penalty on outliers. If set to zero
%       with point to point, the analytical solution will be used
%
% Boundary
%       {[]} | 1x? vector
...
%       usually the fastest. plane will often yield higher accuracy. 
%       Calculation of surface normals requires substantial pre
%       proccessing.
%
% Normals
%       {[]} | n x 3 matrix
...
inp.addParamValue('Lambda', 1, @(x) x >= 0 );
...
for k=1:arg.iter
    ...
    switch arg.Minimize
        case 'point'
            ...
            if arg.Lambda == 0
                [R,T] = EstPose3D(q(:,q_idx),pt(:,p_idx), weights(p_idx));
            else
                [R,T] = EstPose3DL1(q(:,q_idx),pt(:,p_idx), weights(p_idx), arg.Lambda);
            end
        case 'plane'
            weights = arg.Weight(match);
            % [R,T] = eq_plane(q(:,q_idx),pt(:,p_idx),arg.Normals(:,q_idx),weights(p_idx));
            if arg.Lambda == 0
                [R,T] = EstPoseP2Plane(q(:,q_idx),pt(:,p_idx), arg.Normals(:,q_idx), weights(p_idx));
            else
                [R,T] = EstPoseP2PlaneL1(q(:,q_idx),pt(:,p_idx), arg.Normals(:,q_idx), weights(p_idx), arg.Lambda);
            end
    end
